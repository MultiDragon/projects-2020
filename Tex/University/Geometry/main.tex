...
\definition{Порождающий набор векторов} такой набор векторов $w_1,\ldots ,w_n\in V$, что для любого $v\in V$ выполняется $v=\sum w_i\lambda_i$ для какого-то набора $\lambda_1,\ldots ,\lambda_n\in \mathbb F $.

\definition{Линейно независимый набор векторов} такой набор векторов $w_1,\ldots ,w_n$, что из равенства $\sum_i w_i\lambda_i=0$ следует, что $\lambda_i=0\ \forall i$.

\definition{Базис} порождающий линейно независимый набор векторов.

\lemman{о замене} Пусть $w_1,\ldots ,w_n$ порождающий, а $u_1,\ldots ,u_k$ линейно независимы. Тогда $m\geq k$; векторы $w_i$ можно перенумеровать так, что $u_1,\ldots ,u_k,w_{k+1},\ldots ,w_m$ порождают $V$.\label{replacement}
...
\lemman{о базисе} Если существует какой-то конечный порождающий набор, то в любом порождающем наборе есть базис, любой линейно независимый набор можно дополнить до базиса, и в каждом базисе одинаковое количество векторов.\label{basis}
...
\definition{Размерность пространства} количество векторов в его базисе.
...
\definition{Векторное подпространство} такое подмножество $U\subset V$, что $\forall u,v\in U:u+v\in U$ и $\forall u\in U,\lambda\in \mathbb F :u\lambda\in U$.
...
\definition{Сумма подпространств} множество $U+W=\{u+w\mid u\in U,w\in W\}$.
...
\definition{Трансверсальные подпространства} подпространства $U,W$ такие, что $U\cap W=\{0\}$.
...
\theoremn{Полиномиальная интерполяция с кратными узлами} Пусть заданы числа $\alpha_1,\ldots \alpha_n\in \mathbb F $ и для каждого $i$ заданы $m_i+1$ значений: $\beta_{i,0},\ldots ,\beta_{i,m_i}\in \mathbb F $. Пусть $N=\sum_i (m_i+1)$. Тогда существует единственный многочлен $P\in \mathbb F [x]$ степени меньше $N$ такой, что $f^{(k)}(\alpha_i)=\beta_{i,k}$ для всех $i,k$.
...
\definition{Аффинное подпространство} множество $p+U=\{p+u\mid u\in U\}$, где $U$ --- векторное подпространство. По определению, $\dim (p+U)=\dim U$.
